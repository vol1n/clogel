      ...
      (let
        [failure
         (validate-map
          insert-statement
          {:insert
           {:fn #(or (:insertable (:type %)) (:insertable %))
            :message
            "Type provided to insert not insertable not insertable
Make sure there are only assignments and all required fields are there."}
           :unless-conflict {:fn (fn [_] true) :message "should be unreachable"}
         ...
         (validate-map
          ...
          (merge
           {:update
            {:fn #(:updatable (:type %))
             :message
             "Type provided to update not a valid update statement
Make sure there are only assignments."}}
           modifier-validators))]
        (if failure
          failure
          {:type (:type update)
...
(defn with-validator
  ...
  (let [bindings (:with with-entry)
        ...
        errors (keep #(when (not
                             (and (vector? %) (= (count %) 2) (symbol? (first %)) (:type (last %))))
                        {:error/error   true
                         :error/message (str "Malformed with statement: Malformed binding " %)})
                     bindings)]
    (if (empty? errors) rest {:error/error true :error/errors errors})))
  ...
  (let [for (:for for-statement)
    ...
    (if failure
      ...
      (if (or (not for) (not union))
        {:error/error true :error/message ":for and :union required for for statements"}
...
(defn group-by-validator
  ...
  (let [group (:group group-by-statement)
        ...
        failure (validate-map
                 group-by-statement
                 (merge {:group {:fn (fn [_] true) :message "none"}
                         :by    {:fn (fn [_] true) :message "none"}
                         :using {:fn (fn [bindings]
                                       (let [errors (keep #(when (not (and (vector? %)
                                                                           (= (count %) 2)
                                                                           (symbol? (first %))
                                                                           (:type (last %))))
                                                             {:error/error true
                                                              :error/message
                                                              (str "Invalid binding form " %)})
                                                          bindings)]
                                         (if (seq errors) errors true)))}}
                        modifier-validators))]
    (if (not (and group by))
      {:error/message "Group by requires :group and :by" :error/error true}
      (if failure
        failure
